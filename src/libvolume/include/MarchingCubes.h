...
public :
   ...
   */
  MarchingCubes ( vlVolume *vol,float cutoff) ;
  /** Destructor */
  ~MarchingCubes() ;
...
public :
  ...
  inline int size_z() const { return _size_x; }
   ...
   */
  inline float get_data  ( const int i, const int j, const int k ) const { float aux; data->getVoxel(vlPoint3ui(i,j,k),aux);return(aux-_cutoff);};
  /**
   ...
   * \param k height of the cube
   */
  inline void  set_data  ( const float val, const int i, const int j, const int k ) { data->setVoxel(vlPoint3ui(i,j,k),val);};
...
/**
 * VML exportation of the input grid
 * \param fn  name of the ISO file to create
 */
  void writeVMD( const char *fn ,int option=0) ;

// Open Inventor / VRML 1.0 ascii exportation
void writeVRML(const char *fn );
//-----------------------------------------------------------------------------
// Algorithm
public :
